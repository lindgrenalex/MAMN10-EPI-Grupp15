...
Kinect_own::Init()
{
    ...
    output_shortest = GetOutputArray("SHORTESTDISTANCE");
    output_focus_position = GetOutputArray("FOCUS_POSITION");
    output_head_position = GetOutputArray("HEAD_POSITION");
...
Kinect_own::Tick()
{
    ...
    else if(!xtion)// mm, registered
    {
        ...
        output_shortest[0] = shortestDistance;
        output_focus_position[0] = x_position / 640;
        output_focus_position[1] = y_position / 480;
        output_head_position[0] = ((x_position - 320)/10 + 180);
        
        printf("%f at x: %f and y: %f \n", shortestDistance, output_focus_position[0], output_focus_position[1]);
...
static InitClass init("Kinect_own", &Kinect_own::Create, "Source/UserModules/Kinect_own/");
